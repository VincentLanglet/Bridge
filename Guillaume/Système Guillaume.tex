...
\Chap{Ouverture mineure}
	\Section{Soutien mineur inversé}
		...
		&& 4\trefle && Singleton \carreau/\trefle\ (Autre mineur) ; Chelemisant\\
		\end{tabular}\\\\
...
\newpage
		...
		\end{tabular}\\\\
 		...
 		\SSection{Quatrième forcing}
		\begin{tabular}{cccc|l}
		1\trefle & - & 1\coeur & - &\\
		1\pique & - & 2\carreau & - & Quatrième forcing\\
		2\coeur &&&& 3\coeur\ 12-14 (Parfois 2\coeur\ sans autre enchère si le fit est collé)\\
		2SA &&&& Arrêt \carreau\ 12-14 (ou 18-19)\\
		3\trefle &&&& RAS (mais rallonge la couleur au palier de 3)\\
		3\carreau &&&& 15+ 4441 singletion \coeur\\
		3\coeur &&&& 3\coeur 15-17\\
		3SA &&&& Arrêt \carreau\ 15-17 (donc singleton \coeur)\\
		\end{tabular}\\\\
		...
		1\pique & - & 2\carreau & - & \\
		2\coeur & - & 3\carreau & - &\\
		3\coeur &&&& 3\coeur\ sans l'arrêt \carreau\\
		3\pique &&&& 2\coeur\ sans l'arrêt \carreau\\
		3SA &&&& 3\coeur\ avec l'arrêt \carreau\\
		\end{tabular}\\\\
		...
		1\trefle & - & 1\coeur & - &\\
		2\trefle & - & 2\carreau & - & Troisième forcing\\
		2\coeur & - & 2SA && Relais forcing (et vérifiant le fit)\\
		&& 3\coeur &&Chelemisant\\
		\end{tabular}\\
...
\newpage
	\Section{En cas d'interventions}
		...
		\begin{tabular}{cccc|l}
		1\trefle & 1\carreau & X & - & Texas 4-5\coeur\\
		1\coeur &&&& 12-14 ; 3\coeur\ sans l'arrêt (Ne dénie pas 4\pique)\\
		1\pique &&&& 4\pique\ sans 3\coeur\ (avec ou sans l'arrêt \carreau)\\
		1SA &&&& 12-14 ; Peut cacher 3\coeur\ avec l'arrêt \carreau\\
		\end{tabular}\\
		
		Si la place le permet on continue en double-deux sur les enchères de 1\coeur\ et 1SA\\

		\begin{tabular}{cccc|l}
		1\trefle & 1\coeur & X & - & Texas 4-5\pique\\
		1\pique &&&& 12-14 ; 3\pique\ sans l'arrêt\\
		1SA &&&& 12-14 ; Peut cacher 3\pique\ avec l'arrêt \coeur\\
		\end{tabular}\\

		Si la place le permet on continue en double-deux sur les enchères de 1\pique\ et 1SA\\
...
\newpage
		\SSection{2SA Good Bad}
		On joue 2SA Good-Bad dans toutes les séquence où
		\begin{itemize}
		\item Le n°1 ouvre au palier de 1 (possible aussi en majeur)
		\item Le n°3 fait une enchère au palier de 1 (ou X)
		\item Le n°4 fait une enchère au palier de 2\\
		\end{itemize}
		
		\begin{tabular}{cccc|l}
		1\carreau & Passe/1\coeur & 1\pique/X & 2\coeur &\\
		2SA &&&& 18-19 régulier OU 12-14 Unicolore OU 12-14 Bicolore mineur\\
		3\trefle &&&& 15-17 ; Bicolore\\
		3\carreau &&&& 15-17 ; Unicolore\\
		\end{tabular}\\\\
		
		\begin{tabular}{cccc|l}
		1\carreau & Passe/1\coeur & 1\pique/X & 2\coeur &\\
		2SA & - & 3\trefle && Pour jouer en face d'un bicolore mineur\\
		&& 3\carreau && Pour jouer en face d'une première zone\\
		&& Autre && FM\\
		\end{tabular}\\\\
		...
		1\carreau & Passe/1\coeur & 1\pique/X & 2\coeur &\\
		2SA & - & 3\trefle & - &\\
		Passe &&&& 12-14 ; Bicolore mineur\\
		3\carreau &&&& 12-14 ; Unicolore mineur\\
		3\coeur &&&& 18-19 régulier sans l'arrêt \coeur\\
		3SA &&&& 18-19 régulier avec l'arrêt \coeur\\
		\end{tabular}\\\\
		
		NotaBene: fonctionne également sur ouverture majeur.\\
		\begin{tabular}{cccc|l}
		1\coeur & Passe/1\pique & 1SA/X & 2\pique &\\
		2SA & - & 3\trefle & - &\\
		Passe &&&& 12-14 ; Bicolore \coeur/\trefle\\
		3\carreau &&&& 12-14 ; Bicolore \coeur/\carreau\\
		3\coeur &&&& 12-14 ; Unicolore \coeur\\
		3\pique &&&& 18-19 régulier sans l'arrêt \pique\\
		3SA &&&& 18-19 régulier avec l'arrêt \pique\\
		\end{tabular}\\\\
...
\newpage
	\Section{Séquences à 4}
		...
		\item 1\coeur\ Passe 2\coeur\ 3\carreau\ X = Enchère d'essai (Car manque de place)\\
		...
		\item 1\trefle\ Passe 1\pique\ Passe 1SA Passe Passe X = Punitif (entame \pique)
		\item 1\pique\ Passe 1SA Passe Passe X = Punitif (entame \pique)
		\end{itemize}
...
\newpage
		...
		\item 1\pique\ Passe 2\pique\ 2SA = Bicolore mineur
		\item 1\pique\ Passe 2\pique\ 4\trefle/\carreau\ = Bicolore \trefle/\carreau\ et \coeur\\
...
\Chap{Nota bene}
	...
	Sur la collante (demande à la dame), la règle générale est:
	\begin{itemize}
	\item 5 de l'atout (ou 5SA si 5 de l'atout n'est pas possible) = Pas la dame
	\item 6 de l'atout = La dame sans roi (ou un roi non annoncable)
	\item 6 d'une couleur = La dame avec le roi de la couleur nommée
	\item 5SA (si disponible) = La dame avec 2 rois\\
	\end{itemize}

	Sauf:
	\begin{itemize}
	\item 5\coeur/5\pique\ sur une collante à l'atout mineur = La dame avec le roi de \coeur/\pique
	\item 5\pique\ sur une collante à l'atout coeur = La dame avec le roi de \pique
	\item 5\coeur\ sur une collante à l'atout pique = La dame SANS roi\\\\
	\end{itemize}
	
	\Section{Anti-contrôle}
